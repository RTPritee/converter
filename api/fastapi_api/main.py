...
async def convert(file: UploadFile):
    try:
        ...
        for i in range(max_row_count):
            ...
            for key in keys:
                try:
                    ...
                    value = "unknown"  # for invalid or missing value it will provide unknown
                row[key] = value
            rows.append(row)
        ...
        if "gregorian_date" in df.columns:
            df["gregorian_date"] = pd.to_datetime(df["gregorian_date"], errors="coerce").dt.strftime("%m/%d/%Y").fillna("unknown")
